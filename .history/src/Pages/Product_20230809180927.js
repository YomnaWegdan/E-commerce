...
const Product = () => {
  ...
  const {product ,filterProduct , AllProducts  , detail , View , close , setClose , addToCart , addToFavorite} = useAppContext();
  ...
  return (
    ...
    <div className='products container d-flex justify-content-between  '>
    <h3>Products</h3>
    <p>Home - Products</p>
    ...
    {product.map((el)=>{
      return(
        <div className='box' key={el.id}>
            ...
            <img src={el.Img} alt={el.Title}></img>
            <div className='icon'>
            {isAuthenticated? <li onClick={()=> addToCart(el)} ><AiOutlineShoppingCart /></li> :
            <li onClick={()=> loginWithRedirect()} ><AiOutlineShoppingCart /></li>
          }
              <li onClick={()=>View(el)}><BsEye /></li>
              <li onClick={()=> addToFavorite()}><AiOutlineHeart /></li>                                     
            </div>
            </div>
            <div className='detail '>
            <p> {el.Cat}</p>
